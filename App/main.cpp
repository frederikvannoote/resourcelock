...
void priorityLock(int prio)
{
    ...
    // Section where we do something with the resource
    {
        ...
        while (!lock.isValid())
        {
            qInfo() << "[prio" << priority << "] Could not get lock. Will wait for it...";
            QThread::sleep(1);
            lock = resource.lock();
        }

        // Do something with the resource
        qDebug() << "Priority" << priority << "acquired the lock.";
        CancellationRequest cancellation(resource.name() + QStringLiteral("-cancellation-") + QString::number(priority));

        for (int i=0; i<10; i++)
        {
            if (!cancellation.isRequested())
                resource.features().at(0)->doSomething(lock);
            else
            {
...
int main(int argc, char *argv[])
{
    ...
    //priorityLock(parser.value(priorityOption).toInt());
